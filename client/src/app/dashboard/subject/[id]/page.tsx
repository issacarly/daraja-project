...
// ── Real server action — same one SubjectsPage already uses ──
  ...
  useEffect(() => {
    ...
    getAllSubjects()
      .then((res) => {
        ...
        const meta = (res.subjects || []).find((s: any) => s.id === id);
        ...
        // Merge the REAL subject row (name/icon/color/grade) with mock term
        // content looked up by subject name — this is what actually fixes
        // the "every subject shows Mathematics" bug.
        setSubject(buildSubjectContent(meta));
        setIsLoading(false);
      })
      .catch(() => {
        if (!cancelled) {
